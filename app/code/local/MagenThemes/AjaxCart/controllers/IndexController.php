		...
		if($params['isAjax'] == 1){
			...
			try {
				...
				if (!$cart->getQuote()->getHasError()){
                    ...
                    $response['product_id'] = (int) $this->getRequest()->getParam('product');
                    $title = $this->__('%d X %s was added to your shopping cart.', $params['qty'], Mage::helper('core')->htmlEscape($product->getName()));
                    $response['product_info'] = array(
                                                    'image_url' => (string) Mage::helper('mtfastion/image')->getImg($product, 100, 100, 'small_image'),
                                                    'title' => $title
                                                );
