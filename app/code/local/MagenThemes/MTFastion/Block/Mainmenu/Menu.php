...
?>
<?php
class MagenThemes_MTFastion_Block_Mainmenu_Menu extends Mage_Catalog_Block_Navigation
{
    protected $_column;
    protected $_isMomenu;
    protected $_isSmart;
                                                        ...
                                                        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false, $vertical = false)
    {
        ...
        if (Mage::helper('mtfastion')->getCfg('menu/show_if_no_children')) {$showblock = true; }
        ...
        $iconHtml= $catdetail->getData('mtmenu_is_icon');
        if (empty($iconHtml)){
            $iconImage= $catdetail->getData('mtmenu_is_icon_image');
            if (!empty($iconImage)){
    ...
    protected function _getCategoryLabelHtml($category, $level){
        ...
        if ($labelCategory){
            $getLabel = trim(Mage::helper('mtfastion')->getCfg('menu/' . $labelCategory));
            if ($getLabel) {
                if ($level == 0){
