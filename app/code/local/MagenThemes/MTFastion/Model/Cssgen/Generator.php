...
?>
<?php
class MagenThemes_MTFastion_Model_Cssgen_Generator extends Mage_Core_Model_Abstract{
    ...
    protected function _generateStoreCss($design, $storeCode){
        ...
        $filedefault = Mage::helper('mtfastion/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'magenthemes/mtfastion/css/' . $design . '.phtml';
        ...
        try{
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('mtfastion')->__("Template file is empty or doesn't exist: %s", $path) );
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array( 'path' => Mage::helper('mtfastion/cssgen')->getGeneratedCssDir() ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mtfastion')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('mtfastion/cssgen')->getGeneratedCssDir()). '<br/>Message: ' . $gener->getMessage());
